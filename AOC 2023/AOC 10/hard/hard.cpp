...
void ParseMap(const CharacterMatrix& m, Map& map, Pos& start)
{
    ...
    Section& s = map(w, h);
    int & e = s.exits;
    if (s.HasEast(map, w, h)) e+=East;
    if (s.HasWest(map, w, h)) e+=West;
    if (s.HasNorth(map, w, h)) e+=North;
    if (s.HasSouth(map, w, h)) e+=South;

}

int main()
{
    //TextFile f("../../AOC 10/data/hard.txt");
    ...
    for (int h = 0; h < m.Height(); ++h)
    {
        for (int w = 0; w < m.Width(); ++w)
        {
            ...
            if (s.exits == None)
            {
                ...
                for (int i = 0; i < w; ++i)
                {
                    if (map(i, h).exits & North) n++;
                    if (map(i, h).exits & North) s++;
                }
                if ((n & 1) || (s & 1)) inside = true;
                if (inside)
                {
                    inside_count++;
                    cout << "I";
            ...
            else
            {
                ...
                case North + South: cout << "|"; break;
                case East + West: cout << "-"; break;
                case North + East: cout << "L"; break;
                case South + East: cout << "F"; break;
                case North + West: cout << "J"; break;
                case South + West: cout << "7"; break;
                default: cout << "?"; break;
                }
            }
        }
